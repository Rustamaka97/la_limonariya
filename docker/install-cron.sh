...
CRON_RUNOUT="0 16 * * * cd $ROOT && docker compose exec -T api npx tsx src/telegram-runout.ts >> /var/log/limon-runout.log 2>&1"

# Эски limon қаторларини олиб ташлаб, янгиларини қўшамиз (қайта ишга тушса дубль эмас).
(
  crontab -l 2>/dev/null | grep -v 'limon-backup.log' | grep -v 'limon-digest.log' | grep -v 'limon-runout.log' || true
  echo "$CRON_BACKUP"
  echo "$CRON_DIGEST"
  echo "$CRON_RUNOUT"
) | crontab -
